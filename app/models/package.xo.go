...
type Package struct {
	...
	CountryOfBilling  NullString  `json:"country_of_billing"`  // country_of_billing
	CurrencyOfBilling NullString  `json:"currency_of_billing"` // currency_of_billing
	PaymentTerms      PaymentTerm `json:"payment_terms"`       // payment_terms
	Price             NullFloat64 `json:"price"`               // price
	NumberOfMonths    NullInt64   `json:"number_of_months"`    // number_of_months
...
func (p *Package) ByID(db XODB, id uint) error {
	// sql query
	const sqlstr = `SELECT ` +
	...
	return db.QueryRow(sqlstr, id).Scan(&p.ID, &p.Name, &p.CountryOfBilling, &p.CurrencyOfBilling, &p.PaymentTerms, &p.Price, &p.NumberOfMonths)
}
